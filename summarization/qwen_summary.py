...
class QwenSummarizer:
    ...
    def build_track_profile_prompt(self, track_id, segments, track_metadata=None, allowed_objects=None):
        ...
        return (
            ...
            "There is no VideoMAE action classifier in this pipeline; describe only supported activity.\n"
    ...
    def build_track_prompt(self, track_id, segments, track_metadata=None):
        ...
        return (
            ...
            "There is no VideoMAE action classifier in this pipeline; do not invent fixed action labels.\n"
    ...
    def build_scene_prompt(self, event_log, track_payload):
        ...
        return (
            ...
            "There is no VideoMAE action classifier in this pipeline; summarize only supported activity.\n"
    ...
    def build_video_scene_prompt(self, event_log, track_payload):
        ...
        return (
            ...
            "Use the compact structured packet as grounding for track IDs, timestamps, reliable actions, objects, and identity continuity.\n"
            "There is no VideoMAE action classifier in this pipeline; do not invent fixed action labels.\n"
    ...
    def build_window_prompt(self, window_packet):
        ...
        return (
            "You are writing a 5-second surveillance action caption.\n"
            "Each track corresponds to one human subject, and display_name / track_ref should be used when present.\n"
            "Use the structured packet for active track IDs, timestamps, objects, and interactions.\n"
            "There is no VideoMAE action classifier in this pipeline; do not invent fixed action labels.\n"
            "Write 1-3 concise factual sentences about visible or directly supported activity. "
            "If a person's exact action is unclear, say they are present/near the relevant area instead of guessing.\n\n"
            f"Window packet:\n{packet_txt}\n\n"
            "5-second action caption:"
        )
    ...
    def build_qa_prompt(self, question, context_docs, conversation_history=None, has_visual_evidence=False):
        ...
        return (
            ...
            "Each track corresponds to one human subject, but track IDs are internal evidence, not user-facing names. "
            "In the final answer, describe people naturally by visible attributes such as clothing, location, carried items, "
            "or role, for example 'the woman in the green shirt' or 'the person near the dishware'. "
            "Only mention labels like 'Person 1' or raw track IDs if the user explicitly asks for IDs or if there is no "
            ...
            "Use conversation history only for resolving references, not as evidence. There is no VideoMAE action "
            "classifier in this pipeline; answer actions from VLM captions, keyframes, and directly retrieved context.\n"
            f"{visual_instruction}"
            "If the context is insufficient, say so explicitly.\n"
            "Use she/he when the user's wording or visual evidence makes gender presentation clear; otherwise use they "
            "or repeat the visual description. Answer in 3-6 sentences and cite time ranges when available, while keeping "
            "track IDs out of the user-facing answer unless explicitly requested.\n\n"
...
class QwenVLSummarizer:
    ...
    def build_video_scene_prompt(self, event_log, track_payload):
        ...
        return (
            ...
            "for track IDs, timestamps, reliable actions, objects, and identity continuity.\n"
            "There is no VideoMAE action classifier in this pipeline; do not invent fixed action labels.\n"
            "Write 2 short paragraphs that summarize the overall scene, main people, object interactions, and notable changes.\n"
    ...
    def build_window_prompt(self, window_packet):
        ...
        return (
            "You are writing a 5-second surveillance action caption.\n"
            "The images are scene frames sampled from within this short window.\n"
            "Each track corresponds to one human subject, and display_name / track_ref should be used when present.\n"
            "Use images for visible actions and layout. Use the structured packet for active track IDs, timestamps, "
            "object attribution, and interactions.\n"
            "There is no VideoMAE action classifier in this pipeline; do not invent fixed action labels.\n"
            "Write 1-3 concise factual sentences about what the active people appear to be doing. "
            "If a person's exact action is unclear from the frames, say they are present/near the relevant area instead of guessing.\n\n"
            f"Window packet:\n{packet_txt}\n\n"
            "5-second action caption:"
        )
